...
export function DashboardPage() {
  ...
  async function sync() {
    ...
    try {
      ...
      setState({ ...next })
      setSyncMessage(user && configured ? 'Progress synchronized.' : 'Local progress refreshed.')
    } catch {
      setState({ ...getLocalLmsState() })
      setSyncMessage('Cloud synchronization failed. Your local progress remains available on this device.')
    } finally {
      setSyncing(false)
    }
  }
  ...
  const stats = useMemo(() => productionCourses.map((production) => {
    ...
    const percent = production.lessons.length ? Math.round((completedLessons / production.lessons.length) * 100) : 0
  ...
  const overallPercent = overallLessons ? Math.round((overallCompleted / overallLessons) * 100) : 0
  ...
  return <>
    <section className="page-hero dashboard-hero"><div className="container dashboard-hero-grid"><SectionHeading eyebrow="My learning dashboard" title={user ? `Welcome back, ${user.user_metadata?.full_name ?? user.email?.split('@')[0] ?? 'learner'}.` : 'Your progress is saved in local demo mode.'} description={configured ? 'Signed-in progress synchronizes across production courses, quizzes, and devices.' : 'Add Supabase environment variables to enable cross-device accounts. Local course tracking remains active.'} /><div className="dashboard-progress-card"><div className="progress-number">{overallPercent}%</div><div className="progress-track" role="progressbar" aria-valuemin={0} aria-valuemax={100} aria-valuenow={overallPercent}><div style={{ width: `${overallPercent}%` }} /></div><p>{overallCompleted} of {overallLessons} production lessons complete</p><button className="reset-button" type="button" onClick={() => void sync()} disabled={syncing}><RefreshCw size={16} aria-hidden="true" /> {syncing ? 'Synchronizing…' : 'Synchronize progress'}</button>{syncMessage && <p className={syncMessage.includes('failed') ? 'status-message error' : 'status-message success'} role="status" aria-live="polite">{syncMessage}</p>}</div></div></section>

    {continueCourse?.nextLesson && <section className="section section-tight-top"><div className="container dashboard-feature-grid"><article className="dashboard-feature-card"><BookOpenCheck size={28} aria-hidden="true" /><span className="eyebrow">Continue learning</span><h2>{continueCourse.nextLesson.title}</h2><p>{continueCourse.nextLesson.moduleTitle}</p><Link className="button primary" to={`/learn/${continueCourse.production.courseId}/${continueCourse.nextLesson.id}`}>Open next lesson <ArrowRight size={18} aria-hidden="true" /></Link></article><article className="dashboard-feature-card"><Target size={28} aria-hidden="true" /><span className="eyebrow">Assessment status</span><h2>{continueCourse.passedQuizzes} of {continueCourse.production.quizDefinitions.length} passed</h2><p>{continueCourse.production.questionBank.length} source questions support randomized attempts in this course.</p>{continueCourse.finalQuiz && <Link className="button secondary" to={`/quiz/${continueCourse.production.courseId}/${continueCourse.finalQuiz.id}`}>Take final assessment <ArrowRight size={18} aria-hidden="true" /></Link>}</article></div></section>}

    <section className="section section-tinted"><div className="container"><SectionHeading eyebrow="Production courses" title="Track each fully authored learning path" description="Courses are registered through a reusable LMS registry rather than hard-coded page logic." /><div className="production-course-grid">{stats.map(({ production, completedLessons, passedQuizzes, percent, nextLesson }) => { const metadata = courses.find((course) => course.id === production.courseId); return <article className="dashboard-feature-card" key={production.courseId}><span className="course-code">{metadata?.code}</span><h2>{metadata?.shortTitle}</h2><div className="progress-track" role="progressbar" aria-valuemin={0} aria-valuemax={100} aria-valuenow={percent}><div style={{ width: `${percent}%` }} /></div><p>{completedLessons}/{production.lessons.length} lessons · {passedQuizzes}/{production.quizDefinitions.length} assessments passed</p>{nextLesson && <Link className="text-link" to={`/learn/${production.courseId}/${nextLesson.id}`}>Continue course <ArrowRight size={17} aria-hidden="true" /></Link>}</article> })}</div></div></section>

    <section className="section"><div className="container"><SectionHeading eyebrow="Quiz history" title="Highest saved scores" description="Retakes are unlimited. Supabase stores signed-in attempts while the dashboard preserves highest local scores." />{allScores.length ? <div className="assessment-list">{allScores.map(({ quiz, score }) => <Link to={`/quiz/${quiz.courseId}/${quiz.id}`} key={quiz.id}><div><strong>{quiz.title}</strong><small>{quiz.description}</small></div><span className={score >= quiz.passingScore ? 'score-pass' : 'score-review'}>{score}%</span></Link>)}</div> : <div className="empty-dashboard"><BadgeCheck size={34} aria-hidden="true" /><h2>No assessment attempts yet.</h2><p>Complete a first module and take its randomized quiz.</p></div>}</div></section>
