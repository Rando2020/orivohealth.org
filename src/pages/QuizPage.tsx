import { ArrowLeft, BadgeCheck, CircleAlert, RefreshCw } from 'lucide-react'
import { useMemo, useState } from 'react'
import { Link, useParams } from 'react-router-dom'
import { useAuth } from '../context/AuthContext'
import { getQuestionPool, getQuizDefinition, randomizeQuestions } from '../data/apiQuestionBank'
...
export function QuizPage() {
  ...
  const { user } = useAuth()
  const definition = quizId ? getQuizDefinition(quizId) : undefined
  const pool = definition ? getQuestionPool(definition) : []
  ...
  if (!definition || courseId !== 'apis-webhooks-integration') {
    ...
    <>
      ...
      <section className="section section-tight-top">
        <div className="container quiz-layout">
          <main className="quiz-question-list">
            {questions.map((question, index) => {
              ...
              return (
                <article className={submitted ? `quiz-question ${isCorrect ? 'correct' : 'incorrect'}` : 'quiz-question'} key={question.id}>
                  ...
                  <div className="quiz-options">
                    {question.options.map((option) => {
                      ...
                      const correctOption = submitted && question.correctOptionIds.includes(option.id)
                      return (
                        <button
                          className={`${active ? 'selected' : ''} ${correctOption ? 'correct-option' : ''}`}
                          type="button"
                          onClick={() => select(question, option.id)}
                          key={option.id}
                        >
                          <span className="option-marker">{String.fromCharCode(65 + question.options.indexOf(option))}</span>
                  ...
                  {submitted && (
                    <div className="quiz-explanation">
                      {isCorrect ? <BadgeCheck size={20} /> : <CircleAlert size={20} />}
                      <div><strong>{isCorrect ? 'Correct' : 'Review this concept'}</strong><p>{question.explanation}</p></div>
                    </div>
                  )}
                </article>
              )
          ...
          <aside className="quiz-sidebar">
            <div className="sidebar-card sticky-card quiz-progress-card">
              {!submitted ? (
                <>
                  <span className="eyebrow">Attempt progress</span>
                  <div className="progress-number">{answeredCount}/{questions.length}</div>
                  <div className="progress-track"><div style={{ width: `${Math.round((answeredCount / questions.length) * 100)}%` }} /></div>
                  <p>Answer every question before submitting. Questions and option order are randomized for each retake.</p>
                  <button className="button primary" type="button" disabled={answeredCount !== questions.length} onClick={() => void submit()}>Submit assessment</button>
                </>
              ) : (
                <>
                  <span className="eyebrow">Attempt result</span>
