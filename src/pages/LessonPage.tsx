    ...
    <>
        ...
        <article className="lesson-article">
          ...
          <section className="scenario-block">
            ...
            <div>
              ...
              <h2>{activeLesson.scenario.title}</h2>
              <p>{activeLesson.scenario.context}</p>
              <strong>{activeLesson.scenario.decision}</strong>
              <div className="recommended-answer"><ShieldCheck size={19} /><p>{activeLesson.scenario.recommendedApproach}</p></div>
            </div>
          </section>

          <section className="lab-panel">
            <div className="lab-panel-header"><FlaskConical /><div><span className="eyebrow">Guided lab</span><h2>{activeLesson.lab.title}</h2></div></div>
            <p>{activeLesson.lab.brief}</p>
            <div className="lab-columns">
              <div><h3>Instructions</h3><ol>{activeLesson.lab.steps.map((step) => <li key={step}>{step}</li>)}</ol></div>
              <div><h3>Required evidence</h3><ul>{activeLesson.lab.evidence.map((item) => <li key={item}>{item}</li>)}</ul></div>
            </div>
            <details><summary>Open answer guide</summary><ul>{activeLesson.lab.answerGuide.map((item) => <li key={item}>{item}</li>)}</ul></details>
          </section>

          <section className="takeaway-panel">
            <span className="eyebrow">Key takeaways</span>
            {activeLesson.keyTakeaways.map((takeaway) => <div key={takeaway}><BadgeCheck size={18} /><span>{takeaway}</span></div>)}
          </section>
        </article>
        ...
        <aside className="lesson-sidebar">
          <div className="sidebar-card sticky-card">
            ...
            {apiModules.map((module) => (
              <div className={module.id === activeLesson.moduleId ? 'module-nav active' : 'module-nav'} key={module.id}>
                <strong>{module.order}. {module.title}</strong>
                {module.id === activeLesson.moduleId && module.lessons.map((id) => {
                  const item = getApiLesson(id)
                  return item ? <Link className={item.id === activeLesson.id ? 'active' : ''} to={`/learn/${activeLesson.courseId}/${item.id}`} key={item.id}>{item.lessonOrder}. {item.title}</Link> : null
                })}
              </div>
            ))}
          </div>
          <div className="sidebar-card resource-card">
            ...
            {activeLesson.sources.map((source) => <a href={source.url} target="_blank" rel="noreferrer" key={source.url}><span><small>{source.publisher}</small>{source.label}</span><ExternalLink size={16} /></a>)}
          </div>
        </aside>
      </section>
      ...
      <section className="lesson-navigation">
        <div className="container lesson-navigation-inner">
          {navigation.previous ? <Link className="button secondary" to={`/learn/${activeLesson.courseId}/${navigation.previous.id}`}><ArrowLeft size={18} /> Previous lesson</Link> : <span />}
          {navigation.moduleQuiz ? (
            <Link className="button primary" to={`/quiz/${activeLesson.courseId}/${navigation.moduleQuiz}`}>Take module quiz <ArrowRight size={18} /></Link>
          ) : navigation.next ? (
            <Link className="button primary" to={`/learn/${activeLesson.courseId}/${navigation.next.id}`}>Next lesson <ArrowRight size={18} /></Link>
          ) : (
            <Link className="button primary" to={`/quiz/${activeLesson.courseId}/apis-final-assessment`}>Final assessment <ArrowRight size={18} /></Link>
          )}
        </div>
      </section>
    </>
